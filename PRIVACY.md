...
- Avatar Build first sends the prepared portrait to EnConvo's currently configured image provider for a head-only identity edit, then uses that canonical head for mouth-shape generation; generated head and viseme assets are retained locally with the avatar;
- Full Body Studio writes its opening styling prompt from the avatar's own portrait: a downscaled copy of the canonical head is sent to EnConvo's currently configured language model, which returns a wardrobe brief matched to that subject. The brief is cached beside the avatar and reused until the portrait changes or the user asks for a rewrite, so the portrait is not re-sent on every visit. If no vision-capable model is reachable, a neutral built-in preset is used instead and nothing is sent;
- Full Body Studio then sends the canonical head plus the user's treatment, presence, and editable styling prompt to EnConvo's currently configured image provider to create three matched front, right-side, and back plates. Side and back generation also use the approved front plate as a wardrobe and proportion reference. All returned images are segmented locally with macOS Vision, and generated faces are not used as the runtime identity;
- Desktop Motion sends only the references required for the behavior you choose: the generated side body and canonical HD head for Horizon Walk, or the front body, canonical HD head, and any optional pose reference for Edge Idle. Only the selected generated keyframe is then sent to the configured video provider. The optional pose reference is used only for geometry: the server's staged copy is deleted after the job, it is not stored in the motion directory, and it is never published to the live runtime. Returned clips are decoded and alpha-cut locally with macOS Vision;
