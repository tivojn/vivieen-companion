...
- **Full Body Studio** — reference the canonical HD head to generate a matched front, right-side, and back full-body turn-around through EnConvo's current image provider, edit or reset a prefilled tasteful fashion prompt on every generation or regeneration, remove all three backgrounds locally with macOS Vision, and attach the front body beneath the original 1024px animated face rather than replacing its identity.
